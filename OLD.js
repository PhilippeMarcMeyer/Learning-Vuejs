...
var appTodo = new Vue({
  ...
  firestore: {
    workListRaw: db.collection('todo-item')
  },
})
			...
			addItem: function () {
				...
				if(this.toDoTitle !=''){
					if(this.addMode == "add"){
						...
						//this.workList.push(newTodo);
						//this.db.collection("todo-work").add(newTodo);
...
const todoCollection = firebase.firestore().collection('todo-work');

let adder = new fireAdder(todoCollection);

todoCollection.onSnapshot((todosRef) => {
	let store = [];
	...
	appTodo.workListRaw = store;
  });
//toDoFiredb.collection("todo-work").add({"id":155316785,"toDoTitle":"sdsdddddsd","toDoSummary":"dfdfdf","done":false,"order":1553169253455,"parentId":0});
//fireAdd({"id":155316785,"toDoTitle":"Test direct","toDoSummary":"dfdfdf","done":false,"order":1553169253455,"parentId":0});

function add(){
				if(appTodo.toDoTitle !=''){
					if(appTodo.addMode == "add"){
						let id = Date.now();
						let newTodo = { "id" : id, "toDoTitle" : appTodo.toDoTitle, "toDoSummary" : appTodo.toDoSummary, "done" : false, "order" : id, "parentId":0};
						//this.workList.push(newTodo);
						//this.workList.push(newTodo);
...
function storageList(listName){
	...
	toDoFiredb.collection(this.name).add(data)
